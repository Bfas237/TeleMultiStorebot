    ...
    inline_query.answer(
        results=[
            InlineQueryResultArticle(
                id=uuid4(),
                title="Inline query with callback",
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        ...
                        "Edit Only Message",
                        callback_data=b"edit_only_message"
                    ),
                    InlineKeyboardButton(
                        "Edit Only Keyboard",
                        callback_data=b"edit_only_keyboard"
                    )],
                    [ InlineKeyboardButton(
                        "Edit Both",
                        callback_data=b"edit_both"
                    )]
                    ]
                )
            )
...
@app.on_callback_query(b"edit_only_message")
def answer(client, query):
    update = query 
    
    cb = query.data
    
    from_user= query.message.from_user.id
    chat_id = query.message.chat.id
    user_id = query.from_user.id
    data = query.data
    dataid = query.id
    
    edited_message = "This is an edited message... Yea it works :D"
    kb = [[ 
                    InlineKeyboardButton(
                        "Edit Only Keyboard",
                        callback_data=b"edit_only_keyboard"
    ...
    text="{}".format(edited_message),
    reply_markup=reply_markups)
    callback_query.answer("{}".format(edited_message), show_alert=True)
    
@app.on_callback_query(b"edit_only_keybord")
def answer(client, query):
    update = query 
    ...
    callback_query.answer("I am edited :D", show_alert=True)


@app.on_callback_query(b"edit_both")
def answer(client, query):
    update = query 
    ...
    edited_message = "This is an edited message with edited keyboard... Yea it works :D"
